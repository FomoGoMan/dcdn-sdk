...
struct DownloaderTaskOption {
  ...
  void (*Notify)(std::shared_ptr<DownloaderTask> task,
...
public:
  ...
  std::shared_ptr<DownloaderTaskBuffer> Next() { return mNext; }
  void Concat(std::shared_ptr<DownloaderTaskBuffer> buf) { mNext = buf; }
...
public:
  ...
  StatusType Status() const { return mStatus; }
  virtual const DownloaderTaskOption *Option() const { return nullptr; }
  bool IsEnd() const {
    auto st = Status();
...
public:
  ...
  virtual std::shared_ptr<DownloaderTask>
  AddTask(const DownloaderTaskOption *opt) = 0;
